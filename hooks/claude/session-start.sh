...
# Set AGENT_STARTER_SESSION_VERBOSE=1 to also emit last commit, file count,
...
# manifests to .claude/.agent-starter.json (via AGENT_STARTER_FINGERPRINT=1 mode
...
# Used by /setup-agents: AGENT_STARTER_FINGERPRINT=1 session-start.sh > .claude/.agent-starter.json
...
META="${AGENT_STARTER_META:-.claude/.agent-starter.json}"
if [ -f "$META" ]; then
