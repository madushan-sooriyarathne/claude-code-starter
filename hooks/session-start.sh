...
# Set CLAUDE_CODE_STARTER_SESSION_VERBOSE=1 to also emit last commit, file count,
...
VERBOSE="${CLAUDE_CODE_STARTER_SESSION_VERBOSE:-0}"
CONTEXT=""
...
META="${CLAUDE_CODE_STARTER_META:-.claude/.claude-code-starter.json}"
if [ -f "$META" ]; then
...
# Verbose extras (opt-in via CLAUDE_CODE_STARTER_SESSION_VERBOSE=1).
if [ "$VERBOSE" = "1" ]; then
