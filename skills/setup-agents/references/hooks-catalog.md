...
| 2 | `scan-secrets` | `scan-secrets.sh` | `PreToolUse` / `Write\|Edit` | Warns (`ask`, not `deny`) on hardcoded secrets (provider token shapes, DB connection strings with creds, generic secret assignments); ignores env-var references and placeholders | **Always pre-mark** (safety) |
